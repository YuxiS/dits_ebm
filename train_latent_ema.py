...
def get_data(args):
    ...
    # dataset_train = CustomDataset(args, data_root=args.data_root)
    # dataset_test = CustomDataset(args, data_root=args.test_root)
    # dataset_ebm = CustomDataset(args, data_root=args.data_root)
...
def main(args):
    ...
    diffusion = GaussianDiffusion(beta_start=args.noise_start, beta_end=args.noise_end)
    optim = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0)
    optim_sheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optim, T_0=150, T_mult=2, eta_min=1e-6)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x_train, y_train) in tqdm(enumerate(dataloader_train), disable=not accelerator.is_local_main_process):
            ...
            t_train = torch.randint(0, diffusion.timesteps, (x_train.shape[0], ), device=device)
            ...
            with accelerator.autocast():
                ...
                ###############################################################
                t_ebm = torch.randint(0, diffusion.timesteps, (x_ebm.shape[0], ), device=device)
            ...
            if (cur_iter-1) % args.img_every == 0 and accelerator.is_main_process:
                ...
                x_q = diffusion.p_sample_loop(ema, shape)
        ...
        if accelerator.is_main_process and epoch % args.eval_every == 0:
            ...
            model.train()
        accelerator.save(ema.state_dict(), os.path.join(args.save_dir, "last_ckpt.pt"))

if __name__=='__main__':
    parser = argparse.ArgumentParser('Energy Based Model')
    parser.add_argument('--data_root', type=str, default='/remote-home/share/yfsong/imageNet/train2', help='Data dir')
    ...
    parser.add_argument('--noise_start', type=float, default=0.00085)
    parser.add_argument('--noise_end', type=float, default=0.012)
